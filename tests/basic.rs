...
/// Load a test PDF generated by fpdf2: single A4 page (595.28 x 841.89 pt).
///
/// Generate with:
/// ```sh
/// python3 -c "from fpdf import FPDF; p=FPDF(); p.add_page(); p.set_font('Helvetica',size=24); p.cell(200,10,text='Hello'); p.output('/tmp/test.pdf')"
/// ```
fn test_pdf() -> Vec<u8> {
    std::fs::read("/tmp/test.pdf")
        .expect("test PDF not found — see docstring for generation command")
}

...
- **`resvg`/`usvg` 0.47.0 → 0.48.1 and `quick-xml` 0.39.2 → 0.42.0 in `zensvg`** (42693ff). resvg/usvg needed no source change (`tiny-skia` stays 0.12.0). quick-xml 0.42 exposes names and text as `str` rather than `[u8]`, which took three mechanical edits in `zensvg/src/optimize.rs` — `METADATA_ELEMENTS` becomes `&[&str]`, the `<?xml` declaration guard compares string literals, and the whitespace-collapse test iterates `text.bytes()`. Same predicates and same three element names, so the optimizer strips and rewrites exactly what it did before; zensvg's 47-test suite covers it. Also de-truncated `zenjp2`'s `thiserror` `"2.0"` → `"2.0.20"` (same resolved version, lock unchanged).
- **`hayro-jpeg2000` held at 0.3.4** (latest 0.4.0). Not a drop-in: 0.4 restructures decoding so `Image::decode()` takes a `&mut DecoderContext` and returns a `DecodedImage<'_>` of per-component channel data, where 0.3 returned the interleaved `Vec<u8>` that `zenjp2/src/codec.rs` hands to `PixelBuffer::from_vec`. Adapting means new component-interleaving and bit-depth-scaling code inside a JPEG 2000 decoder, which needs verification against real JP2 conformance files rather than a compile check.

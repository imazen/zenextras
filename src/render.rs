...
pub fn render_pages(data: &[u8], config: &PdfConfig) -> Result<Vec<RenderedPage>> {
    ...
    for idx in indices {
        ...
        let settings = compute_render_settings(&config.bounds, page_w, page_h, idx, bg)?;
...
fn compute_render_settings(
    ...
    let final_w = width.unwrap_or_else(|| (page_w * x_scale).floor() as u32);
    let final_h = height.unwrap_or_else(|| (page_h * y_scale).floor() as u32);
